...
typedef struct
{
	...
	scws_t s;           //分词名柄s
	scws_res_t res,head;   //分词结果，字符串指针
	char * table;
} ParserState;
...
*/
	...
	scws_send_text(pst -> s, pst -> buffer, pst -> len);//设定要切分的文本数据。

 /*
 scws_res_t scws_get_result(scws_t s)` 取回一系列分词结果集。
	...
	*/
	...
	(pst -> res) = (scws_res_t)-1;//为什么要-1？即将指针指向分词结果的最后一个字符的前一个，注意最后一个字符为\0
...
zhprs_getlexeme(PG_FUNCTION_ARGS)
{
	...
	if((pst -> res) == (scws_res_t)-1 ){
		...
		(pst -> head) = (pst -> res) = scws_get_result(pst -> s);//将分析器的头指针指向分词结果集的首地址
	}
	...
	if(pst -> res != NULL)
	{
		...
		*t = pst -> buffer + cur -> off;
		...
		if(pst -> res == NULL ){
			...
			scws_free_result(pst -> head);//当头部指针为空时，释放分词器空间
