...
chinese_tokenize_v2(PG_FUNCTION_ARGS)
{
	...
	//PG_RETURN_DATUM(DirectFunctionCall2(testprs_start1,PointerGetDatum(out_txt),len));
...
/*
CREATE OR REPLACE FUNCTION  testprs_start_v2(cstring) RETURNS cstring
AS 'nlpir_parser.dll', 'testprs_start'
LANGUAGE c VOLATILE;

or

CREATE OR REPLACE FUNCTION  testprs_start_v2(cstring) RETURNS int
AS 'nlpir_parser.dll', 'testprs_start'
LANGUAGE c VOLATILE;
...
//	 pst->buffer = (char *) PG_GETARG_POINTER(0);
	...
	//pst->buffer = PG_GETARG_CSTRING(0);
	...
	// NLPIR_Init(sDataPath,1,0);//初始化分词器0,gbk;1,utf8
   ...
   // len = Int32GetDatum(strlen(splitWords));
	...
	// pst->sege= pstrdup(NLPIR_ParagraphProcess(pst->buffer,0));
	 //pst->sege =pstrdup("我 是 中国 人");
	// pst->sege =(char *)DirectFunctionCall1(chinese_tokenize_v2,PointerGetDatum(pst->buffer));
	// pst->len = strlen(pst->sege);	 
	//  pst->len =DirectFunctionCall1(out_len,PointerGetDatum(pst->buffer));
	 // pst->len =DatumGetInt32(DirectFunctionCall1(chinese_tokenize,PointerGetDatum(pst->sege)));
	  // pst->len =(int)(DirectFunctionCall1(chinese_tokenize,PointerGetDatum(pst->sege)));
	 // pst->len =Int32GetDatum(strlen(pst->sege));
	 // pst->len=strlen(splitWords);
	   pst->len = PG_GETARG_INT32(1);//Desc: length of the text 出错在这里啊
	 // pst->len =Int32GetDatum(strlen(pst->buffer));
	 // pst->len =len;
	 // len = Int32GetDatum(strlen(pst->buffer));
	 // len  += pg_mblen(splitWords);//error
	 // pst->len =19;//19,ok 我 是 中国 人 \0	 
	 //   pst->len =8;//我是,即一个汉字=4个字节，utf8
	//pst->len=17;//12/4=3个汉字，即我是中，而结果却是，我是中国
	 // pst->len=Int32GetDatum(strlen(splitWords));
	//   pst->len=Int32GetDatum(strlen(pst->sege));(VARSIZE(in) - VARHDRSZ) / 6;
	//  pst->len =(VARSIZE(pst->sege) - VARHDRSZ);
	...
	// PG_RETURN_INT32(Int32GetDatum(strlen(pst->sege)));
	 // PG_RETURN_INT32(strlen(pst->sege));
	 //PG_RETURN_INT32(len);
	//  PG_RETURN_INT32(pst->len);
	//pfree(splitWords);
	//pfree(in);
	//NLPIR_Exit();//ok
...
testprs_getlexeme(PG_FUNCTION_ARGS)
{
	...
	//char **t=(char **)(pst->sege);
	char	  **t = (char **) PG_GETARG_POINTER(1);
	//int	 *tlen =(int *)(&(pst->len));
